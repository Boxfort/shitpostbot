...
use bot::discord::{Discord, Connection, GetMessages, State};
use bot::discord::model::{Event, ChannelId, MessageId, Message};
use std::env;
...
impl DiscordBot {
    ...
    fn get_all_messages(&mut self, channel : ChannelId, before : MessageId) -> Vec<String> {
        ...
        loop {
            ...
            let result = self.discord.get_messages(channel, GetMessages::Before(id), Some(100));
